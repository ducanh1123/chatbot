import random 
import time
from core.models import Flower, Occasion 
from config import Config

def generate_ai_greeting(flower_name, occasion_name):
    """Mô phỏng việc gọi API AI bên ngoài."""
    
    if Config.EXTERNAL_AI_API_KEY == 'YOUR_SECRET_OPENAI_KEY':
        time.sleep(1) 
        ai_greetings = [
            f"Lời chúc AI Service độc quyền: **Tình yêu rực rỡ như sắc {flower_name} trong ngày {occasion_name}**.",
            f"Lời chúc AI Service cá nhân hóa: **Chúc cho niềm hy vọng và sự tươi sáng mà {flower_name} mang lại sẽ soi đường cho bạn trong mọi chặng đường, đặc biệt là dịp {occasion_name} này.**",
        ]
        return random.choice(ai_greetings)
    else:
        return "Xin lỗi, chức năng AI đang bảo trì."
...
def generate_response(intent, entity):
    """Tạo phản hồi dựa trên dữ liệu từ DB và gọi AI."""
